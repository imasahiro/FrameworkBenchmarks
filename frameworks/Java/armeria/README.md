...
You can create multiple implementations and they will all conform to `[name in benchmark_config.json].dockerfile`. For example, the `default` implementation in `benchmark_config.json` will be `armeria.dockerfile`, but if you wanted to make another implementation that did only the database tests for MySQL, you could make `armeria-mysql.dockerfile` and have an entry in your `benchmark_config.json` for `armeria-mysql`.

5. Test your application

        $ tfb --mode verify --test armeria
...
Edit `.travis.yml` to ensure that Travis-CI will automatically run our verification tests against your new test. This file is kept in alphabetical order, so find where `TESTDIR=Java/armeria` should be inserted under `env > matrix` and put it there.
